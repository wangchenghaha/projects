...
const awards = Config.debug ? Config.awards2.local : Config.awards2.proxy
let _options = {}
Page({
    data: {
    ...
    handleCoupon() {
        ...
        Utils.throttle(() => {
            ...
            isMember(app.config.etoBrand,'DORA_A_DREAM_1',this.data.channel_type).then(res => {
                if (res.errcode == 0) {
                    if (res.is_member) {
                        me.closemodal()
                        me.sendCard()
    ...
    sendCard() {
        ...
        Service.doraHandleCoupon(params).then(res => {
            if (res.errcode == 0) {
                console.log('cardList',res.data.cardList)
                wx.addCard({
                    cardList: res.data.cardList,
                    success: () => {
                        Service.doraCouponStatus(params).then(resp => {
                            if (resp.errcode == 0) {
                                me._home(_options)
                            }
                        })
    ...
    _home(options = {}) {
        ...
        if (options.scene) {
            ...
            params.store_id = Objects.store_id || ''
            params.ctime = Objects.ctime-1591343434 || ''
        } else {
            params.channel_type = options.channel_type || ''
            params.store_id = options.store_id || ''
            params.ctime = options.ctime-1591343434 || ''
        }
        this.setData({
            channel_type :params.channel_type
        })
        console.log('连接入参：', options)
        Service.doraThome(params).then(res => {
            if (res.errcode === 0) {
                this.setData({
                    info: res.data
                })
