import { getExOrderDetail, confirmExchangeOrder, confirmReceiptExOr, calcPrices, culcRemainingTime } from '../ex'
...
Page({
    ...
    onLoad: function (options) {
        ...
        if (DEBUG) {
            ...
            if (res.exchangeType == "STORE") {
                getShopInfo(res.refundShop || res.applyShop)
                    .then(res => {
                        this.setData({
                            shopBean: res,
                        });
    ...
    onExpressBarClick: function (ev) {
        wx.navigateTo({
            url: `../lookDdAdress/lookDdAdress?dingdan_code=${this.data.exDetailBean.expressNo}`
        });
    },
