...
import {
  ...
  judgeUrl,
  dateIsOverDue,
  splitImg, handleCouponDate,  throttle, getQueryStringArgs, } from '../../utils/utils'
...
const app = getApp();
const {brand, cdn, ETO_BRAND, SEARCH_BG, FEED_ID, FULL_SCREEN, title, showCouponTip} = app.config;
let curOptions = {};
let shareTitle = '';
const dmpModule = {
  userLike: 'firstPage',
...
Page({
  ...
  data: {
    ...
    gameIcon: splitImg('icon_game_1029.png', 'common'),
    ...
    newAddCoupon:{
      ...
      etoBrand: brand === 'BESTSELLER'? "" : ETO_BRAND[brand]
    },
    userLikeArr: [],// 猜你喜欢
    hotSaleArr: [], // 热销
    ...
    FULL_SCREEN,
    homePageLoad: true,
    addTipShow: app.globalData.addTipShow,
    addTipIcon: splitImg('icon_add.png', 'common'),
    menuButton: {
      bottom: 60
    }
  },
  ...
  openMemberCard: function () {
    ...
    try {
      this.appTDSdkEvent('pageclick_home_VIPcode', {});
      app.gioTrack('pageclick_home_VIPcode')
    } catch (e) {}
  ...
  getNewAddVoucher: function(){
    ...
    if(CRMInfo){
      const {phone} = CRMInfo;
      const { tip = {} } = app.globalData.configJson;
      // 优先显示配置的弹窗， DB市场部小白
      if(phone && !tip.isShow){
        getVoucher({phone}).then(res => res ? this.handleVoucherData(res) : '')
      }
    }
  },
  ...
  advertisement: function(){
    getConfigJSON().then(res => {
      ...
      const {isHomePageLive, homewishlist = false, tip = {}, homePageHotSearch} = res;
  ...
  getCartList: function () {
    ...
    if(loginInfo){
      ...
      if(localCartList && localCartList.data && localCartList.data.length){
        const cartCount = localCartList.data.length
        app.setCartNum(cartCount);
        if(!FULL_SCREEN){
          this.setData({ shoppingCar:cartCount })
        }
      }
    }
  },
  ...
  getSystem(){
    // 获取右侧胶囊大小
    const {top, height, bottom, right, } = wx.getMenuButtonBoundingClientRect();
    ...
    if(this.data.addTipShow){
      setTimeout( () => {
        this.setData({addTipShow: false})
      }, 10 * 1000)
    }
    if(FULL_SCREEN){
      return
    ...
    query.exec( res => {
      this.setData({mainMarginTop: res[0].height + 10 })
    })
  },
  ...
  advertisementClick: function(){
    let {linkUrl, isJump, videoNumber} = this.data.advertisement;
    if(videoNumber && videoNumber.isShow){
      app.gioTrack('pageclick_home_wxvideo', {
        live_room_no: videoNumber.liveNo,
      });
      app.handleRoomId({live_room_no: videoNumber.liveNo})
      wx.getChannelsLiveInfo({
        finderUserName: FEED_ID,
        success(res) {
          const {feedId, nonceId} = res;
          wx.openChannelsLive({
            finderUserName: FEED_ID,
            feedId,
            nonceId
    ...
    app.gioTrack('pageclick_home_window', {
      popupUrl: linkUrl
    })
    if(isJump){
...
// 企业微信
    ...
    if ("BESTSELLER" === brand || "NAMEIT" === brand) {
      wx.showModal({
        title: '提示',
    ...
    try {
      this.appTDSdkEvent('pageclick_home_shopperbind', {});
    } catch (e) {}

    this.setData({
      guideLoginPopup: true,
    })
  },
  /*
   * 跳转购物车
  ...
  scanning: function () {
    try {
      ...
      app.gioTrack('pageclick_home_search_qrcode')
    } catch (err) {}
    wx.scanCode({
      success: res => {
        res = res.result;
  ...
  onPageScroll: function(e){
    if(throttle(50)){
      ...
      let {goToTop, showFixedTop, addTipShow} = this.data;
      goToTop.isShow = scrollTop >= (screenHeight - 30);
      if(FULL_SCREEN){
        showFixedTop = goToTop.isShow
      }
      if(goToTop.isShow && addTipShow){
        this.setData({addTipShow: false})
      }
  ...
  onShareAppMessage: function (res) {
    ...
    if(isWXWork){
      Object.assign(shareOptions, {utm_source: PAGESTR.wxWorkSource});
    }
  ...
  indexImageShow: function(gameConfigList){
    if(gameConfigList){
      this.setData({games: gameConfigList})
    }
  },
  ...
  gameTrack(url){
    app.gioTrack('pageclick_home_game', {
      url
    })
  },
  // 红包雨游戏
  goGame(e){
    ...
    if(type === 'myPet'){
      ...
      queryUserPoints(app.getOpenId()).then(res =>{
        const url = (res && res.petInfo) ? '/activity/myPet/myPet/myPet' : '/activity/myPet/adoptPet/adoptPet';
        this.gameTrack(url)
        wx.navigateTo({
          url,
        })
