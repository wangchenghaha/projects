  ...
  methods: {
    ...
    onTouchEnd: function(e) {
      ...
      this.setData({
        ...
        hideRemove: false,
        textWidthEnd: this.data.textWidth,
      })
        ...
        case 'handle':
          {
            ...
            let textWidth = this.data.textWidth;
            console.log("diff_x_before=====", diff_x_before);
            console.log("diff_x_after=====", diff_x_after);
            if(diff_x_after < 0){
              return
            }
            if(diff_x_after > diff_x_before){ 
              textWidth =  (textWidth + (diff_x_after - diff_x_before)) < 300 ? textWidth + (diff_x_after - diff_x_before): 300;
            } else {
              textWidth = (textWidth -  (diff_x_before - diff_x_after)) > 30 ? textWidth -  (diff_x_before - diff_x_after) : 30;
            }
    ...
    active: function() {
      ...
      this.setData({
        ...
        textHandleCenterX: this.data.stickerCenterX + 100 * Math.sqrt(2) * this.data.scale * Math.cos((45 + this.data.rotate) * Math.PI / 180),
        textHandleCenterY: this.data.stickerCenterY - 70 * Math.sqrt(2) * this.data.scale * Math.sin((45 + this.data.rotate) * Math.PI / 180),
      })
