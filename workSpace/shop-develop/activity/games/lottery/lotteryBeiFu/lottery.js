...
import {lotteryAdaperBeiFu} from '../lotteryAdapter'
...
Page({
  ...
  startLottery(){
    ...
    let joindate =  wx.getStorageSync(KEYSTORAGE.crmInfo).joindate
    if(lotteryNum < 4){
      if(!this.comperaTime(joindate)){
        json = {
  ...
  comperaTime(startTime){
    ...
    console.log("startTimer+ 1小时=======",startTimer+ 3600000);
    if(currentTime >  startTimer + 3600000){
      return true;
    } else {
      return false;
    }
